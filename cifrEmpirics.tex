...
\section*{Estimating systemic risk measures}

\section{$\sr$}

$\sr$ is a Systemic Risk measure based on the Marginal Expected Shortfall [$\mes$] due to  \cite{acharya2012aer}. $\sr_{jt}$ is the expected capital shortfall of financial institution $j$ conditional on a systemic event
$$\sr_{jt}=\max(0,[kL_{jt}-(1-k)\lmes_{jt}-1]W_{jt})$$
where $k$ is a prudential capital requirement; $W$ is market value of equity; $D$ is book value of debt; $L=(D+W)/W$ is financial leverage; $\lmes$ is long run $\mes$.

$$\lmes_{jt}=E_t(R_{jt+1:t+h}|R_{mt+1:t+h}<C).$$
...
Aggregate systemic risk of financial system is $$\sum_{j=1}^{N}\sr_{jt}.$$
