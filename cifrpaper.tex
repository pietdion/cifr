...
Special cases of the general setup \eref{cusp} or \eref{spline} are set out in \tref{pg}.   In this table, the notation $s\rightarrow\infty$ means $s$ becomes large and $\sigma$ small in such a way that $s\sigma$, the measurement noise in the first equation in \eref{cusp},  stays finite.  In fact $s\sigma\le 1$ since the variance of $z_t$ is 1.  Thus $s\rightarrow\infty$ implies $\sigma\rightarrow 0$ and states that the noise to signal ratio is becoming arbitrarily large. 
With the model the squared correlation between $z_t$ and the signal $\alpha_t+\beta q_t$ is, since the variance of $z_t$ is 1:
 $$
 \rho^2=1-(s\sigma)^2 = 1- \sum_{t=1}^n\{z_t-a(t)-\beta q_t\}^2
 $$
