...
The partially Gaussian  (PG) copula for $(u,v)$ is stated in terms of  $q\equiv\Phi^-(u)$ and $z\equiv\Phi^-(v)$ on the grid  $0<u_1<\cdots<u_n<1$ where $u_t=t/(n+1)$ and 
\be{cusp}
z_t = \alpha_t+\beta q_t+ sh_t\eps_t\cq \alpha_{t+1}=\alpha_t +\delta_t+1.268\eta_t\cq \delta_{t+1}=\delta_t+\eta_t\ .
\ee
Here the $\eps_t$ and $\eta_t$ are serially and contemporaneously uncorrelated zero mean disturbances with common variance $\sigma^2$.  The  $q_t$ term is the   Gaussian component and  $\alpha_t$ a nonparametric deviation.  If $\alpha_t$ and $h_t$ are  identically 0 and  1, respectively, then  $(z_t,q_t)$ with $t$ uniform on $1,\ldots,n$  is bivariate normal  with correlation $\beta=\pm\sqrt{1-(s\sigma)^2}\equiv\rho$.  Comparability between models and fits is facilitated by normalising $h_t$ such that the average weight is 1.

The role of $h_t>0$ in the first equation in \eref{cusp} is to control the smoothness of the fit at different $t$.   If $h_t\approx 0$ then the signal $\alpha_t+\beta q_t$ is close to $z_t$.   If $h_t$ is large then little notice is taken of $z_t$ and the fitted signal at $t$ is relatively smooth.  The $h_t$ are normalised on a  mean of 1.
...
The $\alpha_t=0$ and $\alpha_t=\alpha_0+\delta_0 t$ specifications are generalised by supposing $\alpha_t$  twice differentiable and minimising
\be{spline}
...
with respect to $\alpha_t$ and $\beta$.
 In \eref{spline} the $h_t$  weighs fitting errors: a large  $h_t$ at a particular $t$ implies  error at $t$ is largely ignored while small $h_t$ implies  error at $t$ is  minimized at all cost.  The function $a_t$ and scalar $b$ minimising \eref{spline} deliver a function $a_t+bq_t$ where $a_t$ is piecewise twice differentiable and $a_t+bq_t$ passes closest to , in a mean square error sense, to $z_t$  with a penalty $s^2$ for  roughness measured in terms of the second derivative.   At the minimum, the first term in \eref{spline} is $\sigma^2$. 
...
\cite{bjerve1993correlation} state ``The local regression slope, which focuses on change in expected $y$ as $x$ changes, may be of greater interest than expected  $y$ for a given $x$." They go on to argue for  a  ``universal scale--free standardised version of the local regression slope."  Their suggested measure  in  the current notation is 
\be{rhot}
 \rho_t\equiv\pm\left\{1+\left(\frac{\psi_t}{s\sigma h_t}\right)^{-2}\right\}^{-\frac{1}{2}}\ ,
\ee
where $s$, $\sigma$ and $h_t$ are as in \eref{cusp} or \eref{spline} and $\psi_t$ relates to the slope as outlined below.  The ratio $\psi_t/(s\sigma h_t)$ is a signal to noise ratio and a high value implies $\rho_t\approx \pm 1$.    In general the signal  $\psi_t$ is the local slope of the relationship while $s\sigma h_t$ is the local noise at $t$.   The sign of $\rho_t$ is discussed in the context of particular choices of signal $\psi_t$.  

Take the simplest example -- the Gaussian copula 
$$
z_t=\rho q_t + (\sqrt{1-\rho^2})\varepsilon_t\ ,
$$
where $\varepsilon_t$ is standard normal.  In this  case there is no nonparametric component $\alpha_t$.  The slope with respect to $q_t$ is constant $\psi_t=\rho$ and the noise variance $s\sigma h_t=\sqrt{1-\rho^2}$ constant across $t$ and \eref{rhot} reduces to $\rho_t=\rho$ for all $t$.  This case is displayed in the first row of the of \tref{corr}.
...
\hline
case & $\psi_t$ & $q_t$ & $\alpha_t$ & fitted model/estimate\\
\hline\hline
Gaussian & & & \\
...
The second set of row in \tref{corr} is the nonparametric variation of the Gaussian copula is where, in \eref{cusp},  $\beta=0$.  Hence $q_t$ is eliminated and the  nonparametric component $\alpha_t$ is used to explain the relationship.  The   slope $\alpha_{t+1}-\alpha_t$  at  $t$ is, apart from error
$\delta_t$ suggesting $\psi_t=\delta_t$.   However $\delta_t$ is the slope with respect to $t$ and
\be{normslope}
\psi_t = \frac{\de \alpha_t}{\de q_t} = \left( \frac{\de u}{\de t}\frac{\de q_t}{\de u}\right)^{-1}\frac{\de \alpha_t}{\de t}  \approx
\left\{\frac{1}{n\phi(q_t)}\right\}^{-1}\delta_t  = n\phi(q_t)\delta_t\ ,
\ee
where $\phi$ is the standard normal density.    

The final set of columns in \tref{corr} deals with the general PG case.   Again the overall correlation $\rho$ is derived from the overall fit.   This overall correlation exceeds that derived from both the Gaussian and nonparametric cases.   The higher overall correlation is partialled out locally according to both nonparametric and parametric components.   In any PG fit $\hat\beta$ will differ from the Gaussian copula estimate $\Ex(z_tq_t)$.     
