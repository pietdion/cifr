...
Suppose $y_t$ is a $p$--vector with components $y_{it}$, $i=1,\ldots,p$ with $y_t$ is observed for $t=1,\ldots n$.   Suppose each component series in $y_t$ is ordered according to $x_t$, a scalar series observed or inferred along with the $y_{it}$.   Without loss of generality assume $y_t$, $t=1,\ldots,n$ denotes the component series after ordering with respect to $x_t$ and let $v_t$ and $z_t$ be defined from the componentwise ordered $y_t$ as, for $t=1,\ldots,n$, 
$$
z_t = \Phi^-_*(v_t)\cq v_t = F_*(y_t)\cq q_t=\Phi^-(u_t)\cq u_t=\frac{1}{n+1}\ .
$$
In terms of this notation define the one factor PG model as 
\be{vcusp}
z_t = 1\alpha_t + \beta q_t + s\sigma\{\diag(h_t)\} \eps_t\ ,
\ee
where $\alpha_t$ is  as in \eref{cusp}, and $1$,  $h_t$ and $\eps_t$ are $p$--vectors with component one,  $h_{it}$,  and $\eps_{it}$, respectively.  Further $\beta$ is vector while $s$ and $\sigma$ are scalars.
...
The one--factor PG model is estimated in the same way as the PG model, using vector $z_t$.   In practice $x_t$, the scalar variable serving to order the components of $y_t$, is either external driver or a composition from $y_t$.
