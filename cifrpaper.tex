...
$\ .
With this linear formulation for $\alpha_t$,  $\beta=\rho$ if $\alpha_0=\delta_0=0$.    Model \eref{cusp} with  $\beta=0$ is the called the ``unforced" model since $z_t$ is varies smoothly with $t$, unrelated to $q_t$.  If $\beta=0$ and $\sigma\rightarrow 0$ there is a probit relationship between $z_t$ and $u_t$.  Finally $s=0$ implies $a(t)+\beta q_t=z_t$ for any $\beta$. 
...
From \eref{spline},  $a(t)$ is piecewise twice differentiable passing closest, in a mean square error sense, to $z_t-\beta q_t$  with a penalty $s^2$ for  roughness measured in terms of the second derivative $a''(t)$.  Using observed percentile data $(u_t,v_t)$, $t=1,\ldots,n$, the aim is to estimate $a(t)$ and $\beta$ and associated confidence bounds and assess goodness of fit.   The interpolates of the function $a(t)$ are
 $\E(\alpha_t|z_1,\ldots,z_n)$.   If $s<\infty$ then $a(t)\ne 0$ is thought of as an intercept function. 
 
 \section{Correlational properties of the CG copula}
...
\ee
where the sign is that of $\beta$.  Equation \eref{rho} is equivalent to  
$
