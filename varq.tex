...
\subsection{Theorem}  Suppose $x$ is a random vector with marginal distributions\footnote{To economise on notation, we write $F_j(x)\equiv F_j(x_j)$ and } 
$$
...
By definition
$$
\frac{\de\q(u_i)}{\de u_j} \equiv \q(u_i|u_j>q) -q 
$$
Then $q_{ij}\equiv\q(u|u_j>q)$, is such that
\be\label{Qdef}
