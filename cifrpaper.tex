...
The conditionally Gaussian  (CG) copula for $(u,v)$ is stated in terms of  $q\equiv\Phi^-(u)$ and $z\equiv\Phi^-(v)$ on an arbitrarily fine $u$ grid $0<u_1<\cdots<u_n<1$:  
\be{cusp}
z_t = \alpha_t+\beta q_t+ s\eps_t\cq \alpha_{t+1}=\alpha_t +\delta_t+1.268\eta_t\cq \delta_{t+1}=\delta_t+\eta_t\ .
\ee
Here the $\eps_t$ and $\eta_t$ are serially and contemporaneously uncorrelated zero mean disturbances with common variance $\sigma^2$.  The  $q_t$ term is the   Gaussian component and  $\alpha_t$ a nonparametric deviation.  If $\alpha_t$ is identically zero then the $(z_t,q_t)$  are bivariate normal random variables with correlation $\beta=\pm\sqrt{1-(s\sigma)^2}\equiv\rho$.   Constraint $\alpha_t\equiv 0$ is enforced by simultaneously letting  $s\rightarrow\infty$ and $\sigma\rightarrow 0$ while keeping $s\sigma$ finite. 

The second two equations in \eref{cusp} spell out the model for the nonparametric deviation  $\alpha_t$  with $\alpha_0$ and $\delta_0$ assumed unknown parameters.  If $\sigma\rightarrow 0$  then the first equation in \eref{cusp} becomes
$$
z_t = \alpha_0+\delta_0 t + \beta q_t + s\eps_t \cq t=1,\ldots,n
$$
Averaging both sides over $t=1,\ldots,n$ suggests  $\delta_0=-2\alpha_0/(n+1)$ yielding $
z_t=\alpha_0(1-2u_t) + \beta q_t + s\eps_t$.  In this linear formulation for $\alpha_t$,  $\beta$ is the Gaussian copula correlation only if $\alpha_0=0$.

The zero and linear formulation for $\alpha_t$ is generalised by considering the function $a(t)$ minimising
\be{spline}
...
$$
\end{comment}
The interpolates of the minimising  function $a(t)$ equal $\E(\alpha_t|z_1,\ldots,z_n)$ for \eref{cusp} where the $\eps_t$ and $\eta_t$ are assumed normal and $q_t$ is regarded as fixed.
Note  $a(t)$ is piecewise twice differentiable passing closest, in a mean square error sense, to $z_t-\beta q_t$  with a penalty $s^2$ for  roughness measured in terms of the second derivative $a''(t)$.  Using observed percentile data $(u_t,v_t)$, $t=1,\ldots,n$, the aim is to estimate $a(t)$ and $\beta$ and associated confidence bounds and assess goodness of fit.   The interpolates of the function $a(t)$ are
 $\E(\alpha_t|z_1,\ldots,n)$.
 
 As the roughness penalty $s^2$ increases, $a(t)$ approaches $0$ for all $t$.   In this limiting case $\beta=\pm\sqrt{1-(s\sigma)^2}\equiv \rho$,  the Gaussian copula correlation, since both $z_t$ and $q_t$ are standard normal.  If $s<\infty$ then $a(t)\ne 0$ is thought of as an intercept function.   The partial correlation between $z_t$ and $q_t$ after allowing for $a(t)$ is
\be{rho}
...
\ee
where the sign is that of $\beta$.  As $s\rightarrow\infty$, $(s\sigma)^2\rightarrow 1-\beta^2$ and $\rho_q\rightarrow\rho$.   

In \eref{cusp},  a large $|\beta|$ suggests more extreme tail behaviour than in the Gaussian copula.  To see this note  
 \be{change}
\Delta \E( \alpha_t + \beta q_t)  \approx \frac{-2\alpha_0}{n+1} + \frac{\beta}{(n+1)\Phi'(q_t)}= c\beta\e^{q_t^2/2}\ ,
 \ee
 where $\Delta$ denotes change with respect to $t$ and $c$ is a constant.
Change \eref{change} is relatively large if $q_t$ is in one of the tails and higher than normal tail dependence implies $|\beta|$ large.  For a Gaussian copula, $\beta=\pm\sqrt{1-(s\sigma)^2}$ and comparing the ratio of estimates of the two sides to 1 is a test for Gaussianity.

The  correlation between $z_t$ and $a(t)$ after allowing for $q_t$ is
\be{rho2}
\rho_a\equiv\frac{\cov(z_t-\beta q_t,\alpha_t)}{\sqrt{\cov(z_t-\beta q_t)\cov(\alpha_t)}} = \left\{1-\frac{(s\sigma)^2}{1-\beta^2}\right\}^{1/2}=\left\{\frac{\beta^2/(1-\beta^2)}{\rho_q^2/(1-\rho_q^2)}\right\}^{1/2}\ .
\ee
This compares to the overall correlation between $z_t$ and $a(t)$ of
$$
...
while the nonparametric correlation is the correlation between the nonparametric component $\alpha_t$ and  $z_t$ after removing the the effect of the Gaussian component:
 ...
 The CG is readily  fit to percentile data, and provides a practical platform for copula simulation.  This is discussed, implemented, and used in subsequent sections.  The  parameters defining CG are $s$, $\beta$,  $\sigma$ and starting conditions $\alpha_0$ and $\delta_0$.
...
An example CG copula with $s\sigma=xx$, $\sigma=0$, $\beta=xx$ and $\alpha_0=xx$  is displayed in \fref{xx}.  
 \begin{comment}
...
\section{Correlations with the CG copula}
...
\section{Special cases of the CG copula}
...
\begin{table}[htdp]
\caption{Special cases of the CG copula}\label{pp}
\begin{center}
\begin{tabular}{l|cccccc}
\hline
case&$s$& $\alpha_0$&$\beta$&$s\sigma$\\
\hline
empirical & 0 &  & &$\infty$ &\\
conormal & $>0$ & & $\pm 1$\\
conditionally Gaussian & $>0$ & & $\ne 0$ & $>1$ \\
linear probit & $\infty$ &  &0\\
Gaussian & $\infty$& 0 &$\beta$ & $\sqrt{1-\beta^2}$\\
independence &  $\infty$& 0&0& 1\\
co/counter monotonic & $\infty$ &0 &$\pm 1$& 0 \\
\hline
\end{tabular}
\end{center}
\end{table}%
...
\section{Estimation of the CG model}
