...
The second and third equations in \eref{cusp} spell out the model for the nonparametric deviation  $\alpha_t$  with $\alpha_0$ and $\delta_0$ assumed unknown parameters.  If $\sigma\rightarrow 0$  then the first equation in \eref{cusp} becomes
$$
z_t = \alpha_0+\delta_0 t + \beta q_t + s\eps_t\ .
$$
With this linear formulation for $\alpha_t$,  $\beta=\rho$ if $\alpha_0=\delta_0=0$.  If $\beta=0$, there is a probit relationship between $v_t$ and $u_t$.

The zero and linear formulation for $\alpha_t$ is generalised using function $a(t)$ minimising
\be{spline}
...
$$
\end{comment}
The interpolates of  $a(t)$ equal $\E(\alpha_t|z_1,\ldots,z_n)$ computed with \eref{cusp} where the $\eps_t$ and $\eta_t$ are assumed normal and $q_t$ is regarded as fixed.
From \eref{spline},  $a(t)$ is piecewise twice differentiable passing closest, in a mean square error sense, to $z_t-\beta q_t$  with a penalty $s^2$ for  roughness measured in terms of the second derivative $a''(t)$.  Using observed percentile data $(u_t,v_t)$, $t=1,\ldots,n$, the aim is to estimate $a(t)$ and $\beta$ and associated confidence bounds and assess goodness of fit.   The interpolates of the function $a(t)$ are
 $\E(\alpha_t|z_1,\ldots,z_n)$.  If   $s\rightarrow\infty$ in \eref{spline} then $a(t)$ converges to straight line, while $s=0$ implies $a(t)+\beta q_t=z_t$ for any $\beta$.
...
\ee
where the sign is that of $\beta$.  In other words
$$
\O(\rho_q) =\frac{\beta}{s\sigma} \cq \O(x) \equiv \pm\sqrt{\frac{x^2}{1-x^2}}
$$ 
As $a(t)\rightarrow 0$,  $\rho_q\rightarrow\rho$.  
...
where
$$
\cov(\Delta z_t|t) = \cov(\delta_{t}|t)+(2+3-\sqrt 3)\sigma^2+ \beta^2\cov(\Delta q_t) = c\sigma^2+\beta^2\cov(\Delta q_t)\ ,
$$
with $c=6-\sqrt 3$ and 
$$
\cov(\Delta q_t) \approx (q'_t)^2\cov(u_t)= \frac{\pi\e^{q_t^2}}{6}\ , 
$$
and
$$
\frac{\cov(\Delta z_t)}{\beta^2\cov(\Delta q_t)} =1+ \left(\frac{\sigma s_t}{\beta}\right)^2
\cq
\O(\rho_t) = \frac{\beta}{s_t\sigma }
\cq 
s_t = \sqrt{\frac{6c}{\pi}}\e^{-|q_t|}\ .
$$
